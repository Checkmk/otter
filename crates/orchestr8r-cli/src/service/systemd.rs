...
impl SystemdServiceManager {
    ...
    fn socket_unit_path(&self) -> PathBuf {
        self.unit_dir.join("orchestr8r.socket")
    }

    fn service_unit_path(&self) -> PathBuf {
        self.unit_dir.join("orchestr8r.service")
    }
    ...
    fn write_unit_files(&self) -> anyhow::Result<()> {
        ...
        std::fs::write(
            ...
            format!(
                "[Unit]\n\
                 Description=Orchestr8r daemon socket\n\
                 \n\
                 [Socket]\n\
        ...
        .context("write orchestr8r.socket unit")?;

        std::fs::write(
            self.service_unit_path(),
            format!(
                "[Unit]\n\
                 Description=Orchestr8r workflow automation daemon\n\
                 Requires=orchestr8r.socket\n\
                 After=orchestr8r.socket\n\
                 \n\
                 [Service]\n\
        ...
        .context("write orchestr8r.service unit")?;
...
impl ServiceManager for SystemdServiceManager {
    fn enable(&self) -> anyhow::Result<()> {
        ...
        self.systemctl(&["enable", "--now", "orchestr8r.socket"])?;
        println!("orchestr8r service enabled. The service will start on boot.");
        Ok(())
    }

    fn disable(&self) -> anyhow::Result<()> {
        let _ = self.systemctl(&["stop", "orchestr8r.socket", "orchestr8r.service"]);
        let _ = self.systemctl(&["disable", "orchestr8r.socket", "orchestr8r.service"]);
        ...
        println!("orchestr8r service disabled.");
        Ok(())
    }

    fn start(&self) -> anyhow::Result<()> {
        if self.service_unit_path().exists() {
            return self.systemctl(&["start", "orchestr8r.service"]);
        }
    ...
    fn stop(&self) -> anyhow::Result<()> {
        if self.service_unit_path().exists() {
            return self.systemctl(&["stop", "orchestr8r.service"]);
        }
...
mod tests {
    ...
    fn enable_writes_socket_and_service_units() {
        ...
        assert!(service_content.contains("Requires=orchestr8r.socket"));
